...
function PreemptiveSjf() {
  ...
  // ------------------------------------------------------------------
  // UPDATED: generateGantt to include Ready Queue tracking for Preemptive SJF (SRTF)
  // ------------------------------------------------------------------
  const generateGantt = () => {
    ...
    while (remaining.some((p) => !p.completed)) {
      ...
      // 5. Preemption Check
      const nextCurrent = remaining.reduce((min, p) =>
        p.at <= time && !p.completed && p.remaining < min.remaining
          ? p : min, current // Initialize min to current process for comparison
      );
      ...
      if (current.remaining === 0) {
        ...
        gantt[gantt.length - 1].end = time;
        lastProcessId = null;
      } else if (nextCurrent.id !== current.id) {
        // Preemption occurs because a new process (or another existing one) has a shorter remaining time
        gantt[gantt.length - 1].end = time;
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center mt-8 sm:mt-12 md:mt-16 text-black">
        CPU Process Manager (Preemptive SJF)
      </h2>
          ...
          <div className="flex flex-col justify-center p-3 rounded-md bg-cyan-100 mt-6">
            ...
            <div className="flex items-start justify-center gap-1 w-full overflow-x-auto p-2">
              {ganttData.map((g, idx) => {
                ...
                const readyQueueProcesses = g.queueBefore?.map((proc, index) => {
